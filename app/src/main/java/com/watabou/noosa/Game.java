...
import com.easy_games.pixeldungeon.R;
...
public class Game extends Activity implements GLSurfaceView.Renderer, View.OnTouchListener {
    ...
    {
        return instance.getTracker(TrackerName.APP_TRACKER);
    }
...
//                        mobp.show();
    ...
    public static void showFullscreen(String category, String action, String label) {
        if (Badges.isUnlocked(Badges.Badge.RATE_LATER_NEVER)
                || Badges.isUnlocked(Badges.Badge.RATE_BAD)
                || Badges.isUnlocked(Badges.Badge.RATE_BAD_HEAVY)
                ) {
            if (label!=null){
                if (Badges.isUnlocked(Badges.Badge.RATE_LATER_NEVER))
                    label += " RATE_LATER_NEVER";
                if (Badges.isUnlocked(Badges.Badge.RATE_BAD))
                    label += " RATE_BAD";
                if (Badges.isUnlocked(Badges.Badge.RATE_BAD_HEAVY))
                    label += " RATE_BAD_HEAVY";
                }
            TrackEvent(category, action, label);
            //instance.fullscreen.show();
//            instance.showAdd();

        }
    }

    public static void showFullscreenExtra(String category, String action, String label) {
        if (label!=null && Badges.isUnlocked(Badges.Badge.RATE_BAD_HEAVY)) {
            if (Badges.isUnlocked(Badges.Badge.RATE_BAD_HEAVY))
                label += " RATE_BAD_HEAVY";
            TrackEvent(category, action, label);
            //instance.fullscreen.show();
//            instance.showAdd();
    ...
    {
        if (Badges.isUnlocked(Badges.Badge.RATE_NOW) || Badges.isUnlocked(Badges.Badge.RATE_BAD)) {
           // instance.fullscreen.show();
//            instance.showAdd();
        }
    }
